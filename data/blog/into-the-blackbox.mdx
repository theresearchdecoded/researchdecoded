...
1. The key space K is the set of all poly-sized boolean formulas \phi in n variables $z = (z_1, …, z_n) \in {0, 1}^n. Let \phi(z) be the formula \phi at z.
2. The plaintext, (ind = z, m), where $I := {0, 1}^n$ and z is a bit vector representing boolean values a_2, …, z_n.
3. The predicate $P_n on K \times I$ is defined as

$$
P_n(\phi \in K \backslash \{\epsilon\}, ind = z \in I) := 
\begin(cases)
	1 & \text{if } \phi(z) = 1
	0 & \text{otherwise}
\end(cases)
