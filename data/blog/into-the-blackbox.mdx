...
To understand the inner product predicate, we first need to understand what a "ring" and a "field" are and how they differ. To do that, we need to understand additive and multiplicative inverses in modular arithmetic.

Remember the idea of $Z_n$ from our definition of functional encryption? Let’s consider $Z_5$, which contains the possible results of modulo 5: $Z_5 = \{0, 1, 2, 3, 4\}$.

The **additive inverse** of a number `x` is a number `y` such that `(x + y) mod n = 0`. For example, in $Z_5$, the additive inverse of 1 is 4, because $(1+4) \bmod 5 = 0$.

Similarly, the **multiplicative inverse** is a number `y` such that `(x . y) mod n = 1`. However, not all numbers have a multiplicative inverse. In $Z_8 = \{0, 1, 2, 3, 4, 5, 6, 7\}$, the number 2 has no multiplicative inverse, as no product `(2 . y) mod 8` ever equals 1.

This brings us to rings and fields:
-   A **Ring** is a set like $Z_n$ where multiplicative inverses may not exist for all elements.
-   A **Field** is a set like $F_p$ (where $p$ is a prime number) where every non-zero element *does* have a multiplicative inverse.

Proofs are much simpler in fields. Hence, the inner product predicate is defined over a field $F_p$. The idea is simple: both the key and the index are vectors, and access is granted if their dot product is 0.
